...
-- 20181121 グループチャットでやり取りをしている際にログインメンバーと所属メンバー毎で投稿した順番が分かるようにテーブルを拡張
ALTER TABLE `GROUP_CHAT_EXCHANGE` ADD GROUP_CHAT_EXCHANGE_DISPLAY_NO BIGINT(20) UNSIGNED COMMENT 'グループチャットやり取り表示番号' AFTER GROUP_CHAT_NO;
...
-- 20181121 ダイレクトチャットでやり取りをしている際にログインメンバーとメンバー毎で投稿した順番が分かるようにテーブルを拡張
ALTER TABLE `DIRECT_CHAT_EXCHANGE` ADD DIRECT_CHAT_EXCHANGE_DISPLAY_NO BIGINT(20) UNSIGNED COMMENT 'ダイレクトチャットやり取り表示番号' AFTER GROUP_CHAT_NO;
